...
- [ ] Durable, blind gespeicherte Konten: Identitäts-/Routing-Daten serverseitig persistent, aber für den Server inhaltlich blind (keine Klartext-Identität, keine lesbaren Metadaten) — vom denker zerlegt:
  ...
  - [ ] 0.1c Gruppen-Metadaten (Name/Beschreibung/Avatar) E2EE statt Server-Klartext — verifiziert 2026-06-20: aktuell KLARTEXT auf Server (`/api/groups`, `StoredGroup`); #25 war in CLAUDE.md fälschlich als „shipped" geführt (korrigiert). Umsetzbar via vorhandenem Group-Secret = `GROUP_METADATA_PRIVACY.md` Phase 1.
