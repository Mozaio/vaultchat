...
- [x] Medien-/Datei-Galerie pro Chat (verschlüsselt, nur im Browser). **Umgesetzt**: `ChatShell` berechnet `sharedMediaItems` (client-entschlüsselt aus IDB) und übergibt sie an `InfoPanel` (DM+Gruppe) — geteilte Medien (Datei/Voice, Download) pro Chat, zero-knowledge. **Enhancement offen:** „Alle anzeigen"-Vollgalerie statt nur 8er-Vorschau im Info-Panel.
...
- [ ] Kontakte & Profile: Anzeigename + Avatar, E2E-verschlüsselt geteilt. **Einziger echt offener Phase-1-Punkt** (kein Profil-/Avatar-Editor im Client gefunden). **denker-Ansatz (Signal-Profile-Keys):** pro-User „Profile Key" (32 B) generieren + über den bestehenden Olm-Kanal an Kontakte teilen; Profil (Name+Avatar) client mit dem Profile Key AEAD-verschlüsseln, als Ciphertext serverseitig ablegen (Server sieht nur Blob, analog #25 für Gruppen). ⚠️ Krypto+UI → eigene denker→…→tester-Runde mit Verifikation am laufenden Client (Cloud/Sandbox), NICHT blind bauen.
...
- [ ] Push-Benachrichtigungen ohne Inhalts-Leak an den Push-Dienst (nur „Wakeup", Inhalt wird lokal entschlüsselt). Teilweise: **3-stufige Notification-Privacy** (Name+Inhalt / nur Name / nichts) + Desktop-Notifications existieren (`desktopNotify.ts`, `serviceWorker.ts`). **Zu verifizieren/bauen:** echtes Web-Push (Service-Worker-Push) als reines „Wakeup" ohne Inhalt an den Push-Dienst — prüfen, ob über lokale Notifications hinaus implementiert.
- [ ] Link-Vorschauen clientseitig erzeugt (kein Server-Fetch, kein Leak an Dritte). Wahrscheinlich umgesetzt (`inlineMarkdown.tsx` `extractLinks`/`shortenUrl` rendert Links als Karte/gekürzte URL). **Zu verifizieren:** dass die Zielseite NICHT gefetcht wird (sonst Leak) — Code kurz prüfen, dann abhaken.
