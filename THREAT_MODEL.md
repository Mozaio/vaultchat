...
- **Medien-Pipeline (Bild-Komprimierung + verschlüsselte Thumbnails)**: Bild-Anhänge (JPEG/PNG/WebP > 64 KiB) werden clientseitig vor der Verschlüsselung durch ein Canvas heruntergerechnet (lange Kante ≤ 2048 px, Re-Encode mit Qualität 0.82) und bekommen ein kleines (~256 px) JPEG-**Thumbnail**. Das Thumbnail wird als Feld `thumb` in dieselbe `PlainPayload` gelegt und reist damit **innerhalb** des bereits sealed Envelopes (DM: Sealed-Sender, Gruppe: Megolm) — es ist also **E2EE by construction**: kein zusätzlicher Schlüssel, kein separates Server-Objekt, **kein Dritt-Fetch**. Der Server sieht weiterhin nur Ciphertext. Das Re-Encode über Canvas **entfernt zusätzlich EXIF/GPS-Metadaten** der Originaldatei (Standort/Gerät/Aufnahmezeit), die sonst ungewollt an den Empfänger geleakt wären. Animierte GIFs und SVGs werden bewusst NICHT re-encodet (Canvas würde Frames verlieren bzw. SVG ist eine Skript-Injektions-Fläche) und unverändert gesendet; entschlüsselte Thumbnails laufen vor dem Rendern durch `safeMediaSrc` (nur `data:image/*`). Rückwärtskompatibel: ältere Clients ohne `thumb` rendern weiter das Vollbild aus `body`.
