 ...
 * @returns {Uint8Array}
 */
export function derive_group_identifier(master_key) {
    const ptr0 = passArray8ToWasm0(master_key, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.derive_group_identifier(ptr0, len0);
    if (ret[3]) {
 ...
 * @returns {Uint8Array}
 */
export function derive_group_public_params(master_key) {
    const ptr0 = passArray8ToWasm0(master_key, wasm.__wbindgen_malloc);
    const len0 = WASM_VECTOR_LEN;
    const ret = wasm.derive_group_public_params(ptr0, len0);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
    var v2 = getArrayU8FromWasm0(ret[0], ret[1]).slice();
...
async function __wbg_init(module_or_path) {
    ...
    if (module_or_path === undefined) {
        module_or_path = new URL('zkgroup_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
